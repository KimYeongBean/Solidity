...
contract IndianPoker_1_2_1 {
    ...
    function startGame(uint256 gameId) external onlyActiveGame(gameId) {
        ...
        require(isParticipant, "Only game participants can start the game");
        ...
        // 각 플레이어의 참가비 1칩 징수
        for (uint256 i = 0; i < game.players.length; i++) {
            require(playerChips[game.players[i].playerAddress] >= 1, "Insufficient chips for entry fee");
            playerChips[game.players[i].playerAddress] -= 1;
            game.players[i].chips = playerChips[game.players[i].playerAddress]; // 남은 칩을 게임 내 칩으로 설정
            game.pot += 1; // 팟에 참가비 추가
        }
        ...
        validGameState(gameId)
    {
        ...
        if (player.card == CardValue.ACE && player.chips > 0) {
            uint256 penalty = (player.chips + 1) / 2; // 홀수 처리를 위해 수식 변경
    ...
    function determineWinner(uint256 gameId) internal view returns (address) {
        ...
        address winner;
        CardValue highestCard = CardValue.JOKER; // 가장 낮은 값으로 초기화
    ...
    function exitGame(uint256 gameId) external {
        ...
        // 모든 플레이어가 나갔는지 확인
        bool allPlayersExited = true;
        for (uint256 i = 0; i < game.players.length; i++) {
            if (playerGameId[game.players[i].playerAddress] == gameId) {
                allPlayersExited = false;
                break;
            }
        }
        
        // 모든 플레이어가 나갔으면 게임 비활성화
        if (allPlayersExited) {
            game.isActive = false;
        }
    }
