...
contract IndianPoker_1_2_2 {
    ...
    // 상태 변수
    mapping(address => uint256) public playerChips;                 // 지갑별 보유 칩 (게임 외)
    mapping(uint256 => Game) public games;                          // 게임 ID → Game
    mapping(address => uint256) public playerGameId;                // 지갑별 참여 중인 게임 ID
    mapping(uint256 => mapping(address => bool)) public hasActed;   // (게임 ID, 플레이어) → 이번 라운드 행동 여부
    ...
    modifier onlyOwner(uint256 gameId) {
        ...
        require(msg.sender == games[gameId].players[0].playerAddress, "Only game creator");
        _;
    }
    ...
    function startGame(uint256 gameId) external onlyOwner(gameId) {
        ...
        require(game.phase == GamePhase.WAITING, "Already started");

        game.phase = GamePhase.BETTING;
        game.currentPlayerIndex = 0;
        game.bettingRound = 1;

        // 초기 팟은 0으로 설정 (게임 내 칩은 joinGame 때 이미 설정됨)
        game.pot = 0;
    ...
    function determineWinner(uint256 gameId) internal view returns (address) {
        ...
        address winner;
        CardValue highestCard = CardValue.TWO; // 초기값
    ...
    // 방 나가기(게임 종료 상태에서만 가능)
    function exitGame(uint256 gameId) external {
        Game storage game = games[gameId];
        require(game.phase == GamePhase.FINISHED, "Not finished phase");
        ...
        )
    {
        ...
        // 자신의 카드는 숨기고, 상대방의 카드만 보이도록 수정
        CardValue visibleCard;
        if (playerAddr == msg.sender) {
            visibleCard = CardValue.TWO; // 자신의 카드는 숨김
        } else {
            visibleCard = player.card; // 상대방의 카드는 보임
        }
    ...
    modifier onlyOwnerGlobal() {
        require(msg.sender == owner, "Only owner can call this function");
        _;
    }

    function emergencyWithdraw() external onlyOwnerGlobal {
        payable(owner).transfer(address(this).balance);
    }

    // 게임 강제 종료 (비상시)
    function emergencyEndGame(uint256 gameId) external onlyOwnerGlobal {
        require(gameId > 0 && gameId <= gameCounter, "Invalid game ID");
        Game storage game = games[gameId];
